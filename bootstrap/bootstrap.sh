...
if [ ! -e "$CLAUDE_DIR/skills/omarchy" ]; then
  skip "skills/omarchy no viene en este clon (se excluye del export público)"
elif [ -e "$HOME/.local/share/omarchy/default/omarchy-skill" ]; then
...
if [ ! -d "$SRC_MEM" ]; then
  skip "sin memoria de origen en $SRC_MEM (normal en el export público; vive en el repo privado)"
