...
- Pipeline: detecta Teams/Zoom/Slack, graba wav (Aggregate Device BlackHole + mic Corsair), `whisper-cli` con `~/.whisper-models/ggml-small.bin`, `claude -p`, nota formateada en `~/Notes/Meetings/`; el transcript crudo se queda solo local en `~/.meeting-recorder/transcripts/*.txt` (desde 2026-09-11 ya no se escribe ningún `(transcript).md` en el vault ni notas placeholder cuando falla el write-up) (`YYYY-MM-DD HH-MM Título.md` y `... (transcript).md`), borra el wav.
...
- **Gotcha 2026-09-09:** en modo `corsair` el Aggregate Device sin el Corsair conectado entrega ~2 s de audio y luego ceros digitales (grabación muerta sin error en el log). Diagnóstico rápido: leer el wav en crecimiento con python (`array('h')`, muestras != 0) y probar los índices avfoundation con `ffmpeg -f avfoundation -i :N -t 4`. Fix inmediato: `MIC_SOURCE="mac"` en `bootstrap/machines/D99MFVLWP6.env`, `kill -INT <pid watcher.py>` (para ffmpeg limpio y restaura la salida, no manda a transcribir) y `open -a ~/Applications/MeetingRecorder.app`. Fix de fondo: modo `auto` que detecta el Corsair al iniciar cada grabación.
